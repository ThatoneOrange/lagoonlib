					...
					UserInputService.InputBegan:Connect(function(input)
						if input.UserInputType == Enum.UserInputType.MouseButton1 and ConfigsTable.Hover ~= nil and NewWindow:isMenuOpen() then
							...
							if (action == 'create') then
								...
								if not isfolder(NewWindow.MenuName) then makefolder(NewWindow.MenuName) end
								if not isfile(NewWindow.MenuName..'\\'..configname..'.txt') then
									writefile(NewWindow.MenuName..'\\'..configname..'.txt', data)
								else
									local count = 0
									for index, cfg in pairs(listfiles(NewWindow.MenuName)) do
										local newname = string.split(cfg, '/')[2]
							...
							elseif (action == 'delete') then
								...
								selectedcfg:Destroy()
								ConfigsTable.SelectedConfig = nil
								if isfile(NewWindow.MenuName..'\\'..ConfigsTable.filename) then delfile(NewWindow.MenuName..'\\'..ConfigsTable.filename) end
								ConfigsTable:UpdateConfigList()
							elseif (action == 'overwrite') then
								...
								if selectedcfg == nil then return end
								local data = HttpService:JSONEncode(NewWindow.Elements)
								if isfile(NewWindow.MenuName..'\\'..ConfigsTable.filename) then appendfile(NewWindow.MenuName..'\\'..ConfigsTable.filename, data) end
								ConfigsTable.SelectedConfig = nil
								ConfigsTable:UpdateConfigList()
							elseif (action == 'load') then
								...
								local data = HttpService:JSONDecode(readfile(NewWindow.MenuName..'\\'..filename))
