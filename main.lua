...
function Library:new(options)
	...
	do
		...
		local function adjust()
			...
			NewWatermark["8f"].Size = UDim2.new(0, textSize.X + 15, NewWatermark["8f"].Size.Y.Scale, NewWatermark["8f"].Size.Y.Offset)
		end
	...
	do
		function NewWatermark:SetTitle(text)
			...
			NewWatermark["93"]["Text"] = options.name..' | '..text
		end
	end
	...
	NewWindow:ToggleKeybinds(false)
	...
	do
		...
		local function resizeFrame()
			local cSize = BindsList["83"].AbsoluteContentSize
			local pSize = BindsList["80"].Size
			BindsList["80"].Size = UDim2.new(pSize.X.Scale, pSize.X.Offset, 0, cSize.Y + 10)
			BindsList["82"].Size = UDim2.new(BindsList["82"].Size.X.Scale, BindsList["82"].Size.X.Offset, 0, cSize.Y)
		end
		BindsList["82"].ChildAdded:Connect(resizeFrame)
		BindsList["82"].ChildRemoved:Connect(resizeFrame)
		resizeFrame()
	...
	function NewWindow:NewTab(options)
		...
		function Tab:NewSection(options)
			...
			function Section:Configlist(options)
				...
				function ConfigsTable:assignName()
					if ConfigsTable.configplaceids ~= nil then
						return tostring(table.concat(ConfigsTable.configplaceids, '-'))
					end
				end
				...
				do
					function ConfigsTable:UpdateConfigList()
						...
						for index, cfg in pairs(listfiles(NewWindow.MenuName)) do
						...
						if input.UserInputType == Enum.UserInputType.MouseButton1 and ConfigsTable.Hover ~= nil and NewWindow:isMenuOpen() then
							...
							if (action == 'create') then
								...
								if not isfile(NewWindow.MenuName..'\\configs\\'..configname..'.txt') then
									if ConfigsTable.configplaceids == nil then
										...
										if not isfolder(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()) then makefolder(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()) end
										
										writefile(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()..'\\'..configname..'.txt', data)
									end
								else
									if ConfigsTable.configplaceids == nil then
										...
										local count = 0
										for index, cfg in pairs(listfiles(NewWindow.MenuName..'\\configs\\'..ConfigsTable.assignName())) do
											local newname = string.split(cfg, '/')[2]
											newname = newname:gsub('.txt', '')
											if string.find(string.lower(newname), string.lower(configname)) and #configname > 3 then
												count += 1
							...
							elseif (action == 'delete') then
								...
								if ConfigsTable.configplaceids == nil then
									...
									if isfile(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()..'\\'..ConfigsTable.filename) then
										delfile(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()..'\\'..ConfigsTable.filename)
									end
							...
							elseif (action == 'overwrite') then
								...
								if ConfigsTable.configplaceids == nil then
									...
									if isfile(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()..'\\'..ConfigsTable.filename) then 
										appendfile(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()..'\\'..ConfigsTable.filename, data)
									end
							...
							elseif (action == 'load') then
								...
								local filename = ConfigsTable.filename
								if selectedcfg == nil then return end
								if not isfile(NewWindow.MenuName..'\\'..filename) then return end
								local data
								...
								if ConfigsTable.configplaceids == nil then
									...
									if not isfolder(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()) then makefolder(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()) end
									data = HttpService:JSONDecode(readfile(NewWindow.MenuName..'\\configs\\'..ConfigsTable:assignName()..'\\'..filename))
								end
